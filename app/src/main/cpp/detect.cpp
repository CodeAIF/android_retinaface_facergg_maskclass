...
namespace Face {
    ...
    Detect::Detect(const string &model_path) {
    ...
    void Detect::SetNumThreads(int numThreads){
        num_threads = numThreads;
    }

    void Detect::SetTimeCount(int timeCount) {
        count = timeCount;
    }
    ...
    void Detect::nmsTwoBoxs(vector<Bbox>& boundingBox_, vector<Bbox>& previousBox_, const float overlap_threshold, string modelname)
    {
    ...
    void Detect::nms(std::vector<Bbox> &boundingBox_, const float overlap_threshold, string modelname){
        if(boundingBox_.empty()){
            return;
        }
        sort(boundingBox_.begin(), boundingBox_.end(), cmpScore);
    ...
    void Detect::refine(vector<Bbox> &vecBbox, const int &height, const int &width, bool square){
        if(vecBbox.empty()){
    ...
    void Detect::extractMaxFace(vector<Bbox>& boundingBox_)
    {
        if (boundingBox_.empty()) {
            return;
        }
    ...
    void Detect::PNet(float scale)
    {
        //first stage
        int hs = (int)ceil(img_h*scale);
        int ws = (int)ceil(img_w*scale);
        ncnn::Mat in;
        resize_bilinear(img, in, ws, hs); //errO WAR
...
#define TIMEOPEN 1 //设置是否开关调试，1为开，其它为关
    void Detect::detect(ncnn::Mat& img_, std::vector<Bbox>& finalBbox_){
        img = img_;
        img_w = img.w;
        img_h = img.h;
    ...
    void Detect::detectMaxFace(ncnn::Mat& img_, std::vector<Bbox>& finalBbox) {
    ...
    //void Detect::detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles){
